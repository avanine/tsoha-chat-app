...
def update_thread(thread_id):
    ...
    if thread_to_update and thread_to_update.user_id == session["user_id"]:
        ...
        try:
            sql = text(
                "UPDATE threads SET title = :title, content = :content, last_modified = NOW() WHERE id = :thread_id"
            )
            db.session.execute(
                sql,
                {"title": new_title, "content": new_content, "thread_id": thread_id},
            )
            db.session.commit()
        ...
        return jsonify(
            {"success": False, "message": "You are not authorized to edit this thread"}
        ), 403
